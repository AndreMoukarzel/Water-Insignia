...
func instance_status(name, target):
	...
	var i = 0
	for status in target.status_vector: # Removes old status if same is re-inflicted
		if status.name == name:
